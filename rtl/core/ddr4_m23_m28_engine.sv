...
module ddr4_m23_m28_engine #(
  ...
  always @(posedge clk or negedge rst_n)begin
    if(!rst_n)begin
      ...
      if(req_push)begin req_write_m[req_wp]<=req_write;req_id_m[req_wp]<=req_id;req_addr_m[req_wp]<=req_addr;req_len_m[req_wp]<=req_len;req_size_m[req_wp]<=req_size;req_burst_m[req_wp]<=req_burst;req_tag_m[req_wp]<=free_idx;req_seq_m[req_wp]<=alloc_seq[req_id];req_wp<=req_wp+1'b1;alloc_seq[req_id]<=alloc_seq[req_id]+1'b1;out_v[free_idx]<=1;out_wr[free_idx]<=req_write;out_id[free_idx]<=req_id;out_seq[free_idx]<=alloc_seq[req_id];out_left[free_idx]<=req_len+1'b1;if(req_size>3'd6||req_burst==2'b11)protocol_error<=1;end
