...
module ddr4_m29_native_bridge #(
  ...
  wire [ID_W-1:0] b_id,r_id;
  wire [1:0] b_resp,r_resp;
  wire [DATA_W-1:0] r_data;
  wire [7:0] outstanding_count,command_count;

  // The current AXI front-end supplies one W beat per AW. M29 therefore accepts
  // production write transactions only when AWLEN==0; read bursts remain supported.
  ...
  ddr4_m23_m28_engine #(.ADDR_W(ADDR_W),.DATA_W(DATA_W),.ID_W(ID_W),.TAG_W(TAG_W)) u_engine(
    ...
    .cpl_valid(sched_rsp_wr),.cpl_tag(tag_out),.cpl_rdata(sched_rsp.rdata),.cpl_resp(sched_rsp.resp),
    .b_valid,.b_ready,.b_id,.b_resp,.r_valid,.r_ready,.r_id,.r_data,.r_resp,.r_last,
  ...
  assign sched_wr_empty=!(cmd_valid&&cmd_write&&p_match_found);
  assign sched_rd_empty=!(cmd_valid&&!cmd_write);
  assign cmd_ready=cmd_valid?(cmd_write?(p_match_found&&sched_wr_pop):sched_rd_pop):1'b0;

  wire tag_full,tag_empty;wire [TAG_W-1:0]tag_out;
  wire tag_push=cmd_valid&&cmd_ready;
  wire tag_pop=sched_rsp_wr&&!tag_empty;
  sync_fifo #(.WIDTH(TAG_W),.DEPTH(16))u_tag_fifo(.clk,.rst_n,.wr_en(tag_push),.wr_data(cmd_tag),.full(tag_full),.rd_en(tag_pop),.rd_data(tag_out),.empty(tag_empty));
  assign sched_rsp_full=tag_empty||bridge_rsp_full;

  reg rsp_sel_b;
  always @* begin
    rsp_sel_b=b_valid;
  end
  assign bridge_rsp={rsp_sel_b?b_id:r_id,rsp_sel_b,cmd_addr,rsp_sel_b?{DATA_W{1'b0}}:r_data,rsp_sel_b?b_resp:r_resp,rsp_sel_b?1'b1:r_last};
  assign bridge_rsp_wr=(b_valid||r_valid)&&!bridge_rsp_full;
  assign b_ready=bridge_rsp_wr&&rsp_sel_b;
  assign r_ready=bridge_rsp_wr&&!rsp_sel_b;
endmodule
