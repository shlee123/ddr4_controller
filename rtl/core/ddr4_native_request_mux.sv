...
// request FIFOs and the command scheduler. Only the selected request is
// exposed downstream, so FR-FCFS and the extended timing guard are in the
// functional request path rather than a monitor-only side path.
module ddr4_native_request_mux #(
  ...
  input  logic     clk,
  input  logic     rst_n,
  input  ddr_req_t wr_req_in,
  input  logic     wr_empty_in,
  output logic     wr_pop,
  output ddr_req_t wr_req_out,
  output logic     wr_empty_out,
  input  ddr_req_t rd_req_in,
  input  logic     rd_empty_in,
  output logic     rd_pop,
  output ddr_req_t rd_req_out,
  ...
  always_comb begin
    ...
    req_write = 2'b01;
    req_bank[0] = wr_req_in.addr[5 +: BANK_W];
    req_bank[1] = rd_req_in.addr[5 +: BANK_W];
    req_row[0] = wr_req_in.addr[9 +: ROW_W];
    req_row[1] = rd_req_in.addr[9 +: ROW_W];
  ...
  ddr4_scheduler_v2 #(.ENTRIES(2), .BANK_W(BANK_W), .ROW_W(ROW_W), .AGE_W(8)) u_native_sched (
    ...
    .open_valid, .open_row, .prefer_writes, .grant_accept,
    .grant_valid, .grant_index, .grant_row_hit, .grant_write, .grant_bank);
  ...
  ddr4_timing_ext #(.T_WTR(2), .T_RTP(2), .T_WR(3), .T_CCD_L(3),
    ...
    .allow_rd, .allow_wr, .allow_pre, .allow_mrs, .allow_zqcs(allow_zq),
